            ...
            if (web3eth.givenProvider) {
                ...
                if (IDByAddress.bankId == 0) {
                    ...
                    let response = await callContract.methods
                        .addClient(IDByAddress.bankId, branchid, clientaddress, amount * 100000000, "EUR")
                    ...
                    let response = await callContract.methods
                        .addClient(IDByAddress.bankId, branchid, clientaddress, amount * 100000000, "USD")
                        .send({ from: address, gas: 1000000 })
                    console.log(response)
        ...
        try {
            ...
            if (web3eth.givenProvider) {
                ...
                if (IDByAddress.bankId == 0) {
                    ...
                    console.log("IDByAddress.bankId, IDByAddress.branchId, forexDetials.clientId, forexDetials.reqId", IDByAddress.bankId, IDByAddress.branchId, forexDetials.clientId, forexDetials.reqId);
                    let response = await callContract.methods
                    ...
                    let response = await callContract.methods
                        .sendForexRequestToBank(IDByAddress.bankId, IDByAddress.branchId, forexDetials.clientId, forexDetials.reqId)
                        .send({ from: address, gas: 1000000 })
        ...
        try {
            ...
            if (web3eth.givenProvider) {
                ...
                if (IDByAddress.bankId == 0 && forexDetials.isApproved) {
                    ...
                    let responseEcb = await callContractECB.methods.approve(contractAddressbnksys, requestDetails.amountInEur).send({ from: address, gas: 1000000 });

                    let response = await callContract.methods
                        .processForexRequestByBranch(IDByAddress.bankId, IDByAddress.branchId, IDByAddress2.bankId, IDByAddress2.branchId, forexDetials.reqId)
                        .send({ from: address, gas: 1000000 })
                    ...
                    setTokenSymbol("USD")
                    let responseFed = await callContractFED.methods.approve(contractAddressbnksys, requestDetails.amountInUsd).send({ from: address, gas: 1000000 });
        ...
        try {
            ...
            if (web3eth.givenProvider) {
                ...
                if (IDByAddress.bankId == 0) {
                    ...
                    let response = await callContract.methods
                        .processLoan(positionDetails.bankId, positionDetails.branchId, positionDetails.clientId, positionDetails.positionId)
                        .send({ from: address, gas: 1000000 })
                    ...
                    <Card>
                        ...
                        <Card.Content extra>
                            <div className='ui two buttons'>
                                {isconnectbuttonclicked ? <></> : <Button secondary onClick={checkDetails}>Connect</Button>}
                            </div>
                        </Card.Content>
                    </Card>
                                ...
                                arrayDataForexDet.map((data, index) => {
                                    ...
                                    return (
                                        <Table.Row key={index}>
                                            ...
                                            <Table.Cell>{data.amountInUsd / 10e7} {data.tokenSymbol}</Table.Cell>
                                            <Table.Cell>{data.amountInEur / 10e7} {data.tokenSymbol}</Table.Cell>
                                            <Table.Cell>{data.fromBranchId}</Table.Cell>
                                            <Table.Cell>{data.toBranchId}</Table.Cell>
                                            <Table.Cell>{data.isSentToBank ? 'True' : 'False'}</Table.Cell>
                                            <Table.Cell> <Button basic color='green' onClick={processForexRequestBranch}>
                                                Approve
                                            </Button>
                                                <Button basic color='red'>
                                                    Decline
                            ...
                            return (
                                <Card>
                                    <Card.Content>
                                        ...
                                        <Card.Header>Borrow Request {0} </Card.Header>
                                        <Card.Meta>Amount {100} USD</Card.Meta>
                                        <Card.Description>
                                            Interest Rate {10} %
                                        </Card.Description>
                                    </Card.Content>
                                ...
                                arrayDataBorrowDet.map((data, index) => {
                                    ...
                                    return (
                                        <Table.Row key={index}>
                                            ...
                                            <Table.Cell> <Button basic color='green'>
                                                Approve
                                            </Button>
                                                <Button basic color='red'>
                                                    Decline
